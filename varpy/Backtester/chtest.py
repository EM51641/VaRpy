...
def POF(total_violations, prob, total_points):
    A = 2 * (total_points - total_violations) * np.log(
        1 - prob
    ) + 2 * total_violations * np.log(prob)
    B = 2 * (total_points - total_violations) * np.log(
        1 - (total_violations / total_points)
    ) + 2 * total_violations * np.log(total_violations / total_points)
    return -A + B
...
def TUFF(Matrix, total_violations, prob):
    ...
    B = 2 * (1 / inter_time_counter) + (inter_time_counter - 1) * np.log(
        1 - 1 / inter_time_counter
    )
    return -A + B
...
def TBFI_test(Matrix: np.ndarray, prob: np.float64):
    ...
    for i in range(0, Matrix.size - 1):
        if Matrix[i] == 0:
            ...
            B = +2 * 1 / (i - inter_time_counter) + (
                i - inter_time_counter - 1
            ) * np.log(1 - 1 / (i - inter_time_counter))
...
def boostrap_loop(sample, Quantile, N):
    ...
    for i in prange(0, N):
        ...
        t_stat[i] = (np.median(x) - Quantile) / (
            np.std(x) / np.sqrt(252)
        )  # We adjust for skewness and kurtosis by choosing the median
    return t_stat
