...
def Student_VaR(return_matrix, theta, Horizon):  # 500 datas needed
    ...
    mean_forecast, var_forecast, conditional_volatility = Arch_data(
        return_matrix, Horizon, "studentst"
    )
    innovations = Extract_Excess_Innovations(
        return_matrix, mean_forecast, conditional_volatility
    )
...
def Var_CVaR_extractor(mean_forecast, var_forecast, alpha, theta):
    unconditional_VaR = np.sqrt((alpha - 2) / alpha) * scipy.stats.t.ppf(
        1 - theta, alpha
    )
    VaR = -(mean_forecast + np.sqrt(var_forecast) * unconditional_VaR)
    CVaR = -(
        mean_forecast
        + (scipy.stats.t.pdf(scipy.stats.t.ppf(1 - theta, alpha), alpha) / theta)
        * (alpha + scipy.stats.t.ppf(1 - theta, alpha) ** 2)
        / (alpha - 1)
